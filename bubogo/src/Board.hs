...
tryCapture b capturedColor c = mutBRead b c >>= \v ->
...
orM :: [IO Bool] -> IO Bool
orM [] = return False
orM (x:xs) = x >>= \r -> if r then return True else orM xs
anyM :: (a -> IO Bool) -> [a] -> IO Bool
anyM f xs = orM (map f xs)
chainCoords :: MutBoardOf Bool -> IO [Coord]
chainCoords chain = catMaybes <$> mapM isIn allCoords where
  isIn c = mutBRead chain c >>= \r -> return (if r then Just c else Nothing)
...
class ShowSq a where showSq :: a -> Char
instance ShowSq Color where showSq Black = '●'; showSq White = '℗'
