...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    case 'o':
      for (format = 0; format < SND_PCM_FORMAT_LAST; format++) {
        ...
        if (format_name && !strcasecmp(format_name, optarg)) break;
      }
      if (format == SND_PCM_FORMAT_LAST) format = SND_PCM_FORMAT_S16;
  ...
  if (samples == NULL) {printf("No enough memory\n"); exit(EXIT_FAILURE);}
  areas = calloc(channels, sizeof(snd_pcm_channel_area_t));
  if (areas == NULL) {printf("No enough memory\n"); exit(EXIT_FAILURE);}
  for (chn = 0; chn < channels; chn++) {
